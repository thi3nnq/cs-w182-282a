...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        if len(queries.size()) == 4:
          similarity = th.matmul(queries, keys.transpose(2, 3)) # Compute the similarity according to the QKV formula
        else:
          similarity = th.matmul(queries, keys.transpose(1, 2))       	
        ...
        weights = F.softmax(masked_similarity*1/(key_dim ** (1/2)), dim=-1) # Turn the similarity into a normalized output. Remember that the last dim contains the features
